...
def duration_stats(df):
    ...
    text_info = "The longest track: {0}, The shortest track: {1}, The average track duration: {2}".format(
        max_var, min_var, mean_var
    )
    return text_info
...
def songs_popularity_bar(df=None):
    hover_names = [
        ", ".join(df["artist_name"][ind]) + " - " + song
        for ind, song in enumerate(df["song_name"])
    ]
...
def song_features_plot(df=None):
    ...
    for col in columns:
        fig.add_trace(
            go.Scatter(
                x=df.index, y=df[col], mode="lines+markers", name=col, text=hover_names
            )
        )
    return fig
...
def keys_piechart(df=None):
    ...
    songs_per_key = [
        f"Key{c[0]} - {c[1]} songs" for c in songs_per_key.items()]
...
def date_song_added(df=None):
    if len(df["added_at_date"].unique()) > 1:
        ...
        fig_d.update_xaxes(
            type="date", range=[values_d.min(), values_d.max()], title="Date"
        )
...
def hour_song_added(df=None):
    if len(df["added_at_date"].unique()) > 1:
        values_h, counts_h = np.unique(
            [pd.Timestamp(1970, 1, 1, h.hour) for h in df["added_at_hour"]],
            return_counts=True,
        )
        fig_h = px.bar(x=values_h, y=counts_h, color=counts_h)
        fig_h.update_xaxes(
            type="date", range=[0, 86400000], title="Hour", tickformat="%H:%M"
        )
