...
def data_generator(playlist_id: str):
    ...
    for song in songs_info_from_playlist:
        ...
        if len(song["artistID"]) > 1:
            genres_list_list = [
                get_api_artist(artist, TOKEN) for artist in song["artistID"]
            ]
    ...
    df["added_at_hour"] = df["added_at_hour"].apply(
        lambda x: datetime.strptime(str(x).replace("Z", ""), "%H:%M:%S").time()
    )
