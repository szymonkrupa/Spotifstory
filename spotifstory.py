...
from dashboard import most_popular_song, least_popular_song, aggregated_features, most_common_genre,\
                    most_common_artist, songs_popularity_bar, song_features_plot, keys_piechart, \
                    date_song_added, hour_song_added, songs_duration_bar, duration_stats, song_tempo_plot
...
with header:
    ...
    col_b.text('See what story playlist wants to tell you about herself!\nMore info on the sidebar.*')
    col_b.markdown('___')

with playlist_link:
    buffer_one, use_info, buffer_two = playlist_link.columns([1,2,1])
    link_data = use_info.text_input('spotify playlist link', "https://open.spotify.com/playlist/37i9dQZEVXbNG2KDcFcKOF?si=2a8def5df7ce465c")
...
with dashboard:
    if use_info.button('Play'):
        ...
        dashboard.plotly_chart(songs_popularity_bar(data), use_container_width=True)

        dashboard.subheader('Duration of the songs:')
        dashboard.write(duration_stats(data))
        dashboard.plotly_chart(songs_duration_bar(data), use_container_width=True)

        dashboard.subheader('Features of the songs:')
        dashboard.plotly_chart(song_features_plot(data), use_container_width=True)
...
st.sidebar.write("To know how to get spotify playlist link, check out this 🌐: [link](https://tutstake.com/2020/08/spotify-playlist-link.html)")
st.sidebar.write("*Playlist can not be longer than 100 tracks.")
st.sidebar.write("📱If you are using a smartphone, I recommend using it horizontally.")
st.sidebar.markdown('___')
    #Features description
st.sidebar.subheader('Features description:')
st.sidebar.write("🎼Key - the estimated overall key of the track. Integers map to pitches using standard Pitch Class notation.\
     E.g. 0 = C, 1 = C♯/D♭, 2 = D, and so on. If no key was detected, the value is -1.")
st.sidebar.write("🪕Acousticness - a confidence measure from 0.0 to 1.0 of whether the track is acoustic. 1.0 represents high confidence the track is acoustic.")
st.sidebar.write("🤸Danceability - describes how suitable a track is for dancing based on a combination of musical elements including tempo,\
     rhythm stability, beat strength, and overall regularity. A value of 0.0 is least danceable and 1.0 is most danceable.")
st.sidebar.write("⚡Energy - a measure from 0.0 to 1.0 and represents a perceptual measure of intensity and activity. \
...
st.sidebar.write("🎛️Instrumentalness - predicts whether a track contains no vocals. “Ooh” and “aah” sounds are treated as instrumental in this context.\
...
st.sidebar.write("📢Loudness - the overall loudness of a track in decibels (dB). Loudness values are averaged across the entire track \
    and are useful for comparing relative loudness of tracks. Loudness is the quality of a sound that is the primary psychological correlate of physical strength (amplitude). Values typical range between -60 and 0 db.")
st.sidebar.write("🗣️Speechiness - detects the presence of spoken words in a track. The more exclusively speech-like the recording\
     (e.g. talk show, audio book, poetry), the closer to 1.0 the attribute value. Values above 0.66 describe tracks that are probably made entirely of spoken words.\
...
st.sidebar.write("😊Valence - a measure from 0.0 to 1.0 describing the musical positiveness conveyed by a track. Tracks with high valence sound more positive\
     (e.g. happy, cheerful, euphoric), while tracks with low valence sound more negative (e.g. sad, depressed, angry).")
st.sidebar.write("💨Tempo - the overall estimated tempo of a track in beats per minute (BPM). In musical terminology,\
     tempo is the speed or pace of a given piece and derives directly from the average beat duration.")
st.sidebar.write("🤘Popularity - the popularity of the track. The value will be between 0 and 100, with 100 being the most popular.\
